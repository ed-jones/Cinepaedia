...
function search(action, id) {
  ...
  if (request) {
    ...
    if (action == search) {
      request.open("GET", "http://www.omdbapi.com/?apikey=" + window.apiKey  + "&r=xml&type=movie&s=" + query, true);
    } else {
      request.open("GET", "http://www.omdbapi.com/?apikey=" + window.apiKey  + "&plot=long&r=xml&i=" + id, true);
    }
    request.send("");
  } else {
...
function parseXMLid(response, id) {
  ...
  if (response) {
    ...
    document.getElementById('entry').innerHTML = "<img onClick='javascript:closeWindow()' id='close' src='images/close.svg'><div id='text'></div>" + "<img id='poster' src='http://img.omdbapi.com/?apikey="+window.apiKey+"&i=" + id + "' onerror='this.src=\"images/notfound.jpg\"''>";
